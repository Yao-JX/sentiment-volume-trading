...
| 項目 | 結果 |
|---|---|
| Random Forest 測試集 ROC-AUC | **0.561**（交叉驗證 0.553，皆優於隨機的 0.5 與 Logistic Regression 的 0.533） |
| 回測總報酬（策略 vs 買進持有） | **66.7% vs 73.9%** |
| 夏普值 Sharpe（策略 vs 買進持有） | **2.26 vs 1.46** |
| 最大回撤 Max DD（策略 vs 買進持有） | **-10.1% vs -30.5%** |
| 做多日勝率 / 做多天數 | 53.4%　/　146 天（測試集共 339 個交易日，其餘空手避險） |

我自己的結論是：情緒聲量對隔日「方向」只有很弱、但確實存在的預測力（ROC-AUC 只比 0.5 高一點點）。它真正有價值的地方在風險控管跟擇時避險——策略總報酬（66.7%）雖略低於買進持有（73.9%），但因為模型在情緒轉差時讓策略空手、躲過大跌，最大回撤從買進持有的 -30.5% 收斂到 -10.1%、夏普值幾乎翻倍（2.26 vs 1.46）。也就是說，它用更小的風險換取了接近的報酬。另外分群也看到一個有趣的現象：聲量爆量反而是個反向指標，爆量那群的隔日上漲率（46.0%）是三群裡最低的。
