...
| 項目 | 結果 |
|---|---|
| Random Forest 測試集 ROC-AUC | **0.576**（交叉驗證 0.550，皆優於隨機的 0.5 與 Logistic Regression 的 0.535） |
| 夏普值 Sharpe（策略 vs 買進持有） | **2.84 vs 1.46** |
| 最大回撤 Max DD（策略 vs 買進持有） | **-11.4% vs -30.5%** |
| 回測總報酬（策略 vs 買進持有） | 116.9% vs 73.9%（**此數字不穩定，見下方說明**）|
| 交易筆數 / 做多天數 | 78 筆進場　/　做多 143 天（測試集共 339 個交易日，其餘空手避險）|

我自己的結論是：情緒聲量對隔日「方向」只有很弱、但確實存在的預測力（ROC-AUC 只比 0.5 高一點點）。它真正有價值的地方在**風險控管**——不管哪一次執行，策略的夏普值都遠勝買進持有（2.84 vs 1.46）、最大回撤也大幅縮小（-11.4% vs -30.5%），因為模型會在情緒轉差時讓策略空手、躲過大跌。另外分群也看到一個有趣的現象：聲量爆量反而是個反向指標，爆量那群的隔日上漲率（46.0%）是三群裡最低的。

> ⚠️ **關於總報酬**：因為這個訊號很弱，回測「總報酬」對資料切分與模型選擇非常敏感（不同執行落在約 67%～117% 之間），所以**不該把總報酬當成穩定結論**；真正穩健、每次執行都成立的是風險調整後的表現（Sharpe、最大回撤）。為了讓結果可重現，價格資料已快取為 `price_2330.csv`，固定在上述這一版。
